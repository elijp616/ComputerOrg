...
struct ascii_image *create_image(int height, int width, char *name) {
    ...
    struct ascii_image* myImage = (struct ascii_image*) malloc(sizeof(struct ascii_image));
    if (!myImage) {
    	free(data);
    	if (myName){
    		free(myName);
    ...
    return myImage;    // Don't forget to replace this!
}
...
int add_extension(struct ascii_image *image, char *extension) {
	...
	char* nameNew = (char*) malloc(length + strlen(extension) + 1);
	if (! nameNew) {
		return FAILURE;
	}
	strncpy(nameNew, name, length);
	strncat(nameNew, extension, strlen(extension));
	image->name = nameNew;
	free(name);
